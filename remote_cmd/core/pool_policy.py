...
v2.6（P2.1）起 canonical 位置为 ``remote_cmd.core.pool_policy``：该模块
只被 core 连接池消费，放在 core 可保持 ``core`` 对 ``service`` 的零依赖。
旧导入路径 ``remote_cmd.service._pool_policy`` 仍可用（re-export shim）。
